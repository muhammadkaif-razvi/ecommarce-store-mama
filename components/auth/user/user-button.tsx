    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="relative flex items-center justify-center w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 transition">
          <Avatar className="w-9 h-9">
            ...
            <AvatarFallback className="bg-gray-700 flex items-center justify-center w-full h-full text-white text-sm">
      ...
      <DropdownMenuContent className="bg-white shadow-lg rounded-lg p-2 w-48">
        {/* Navigation Links (Only Visible on Small Screens) */}
            <DropdownMenuItem className={`p-2 text-gray-800 hover:bg-gray-100 rounded-md transition ${pathname === "/settings" ? "bg-gray-200" : ""}`}>
              <Link href="/settings">Settings</Link>
            </DropdownMenuItem>
            <DropdownMenuItem className={`p-2 text-gray-800 hover:bg-gray-100 rounded-md transition ${pathname === "/profile" ? "bg-gray-200" : ""}`}>
              <Link href="/profile">Profile</Link>
            </DropdownMenuItem>
            {user?.role === "ADMIN" && (
              <DropdownMenuItem className={`p-2 text-gray-800 hover:bg-gray-100 rounded-md transition ${pathname === "/admin" ? "bg-gray-200" : ""}`}>

...
export const PriceForm: React.FC<FormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (initialData) {
        ...
        await axios.post(`/api/${params.storeId}/prices`, values);
      }
      router.refresh();
      update();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/prices/${params.priceId}`);
      router.refresh();
      update();
      router.push(`/${params.storeId}/prices`);
      ...
      toast.error(
        "Make sure you removed all products using this price first."
      );
    } finally {
      setLoading(false);
      setOpen(false);
